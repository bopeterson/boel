...
function handleCakePieceTouch(event) {
	...
	switch (piecenumber) {
		case 0:
			moveCakePiece(0,-80,280,"piece"); //xxx used also in handleTable_0002_dogTouch, coordinates must be moved to central place
			break;
		case 1: 
			moveCakePiece(1,270,200,"piece");
			break;
		case 2:
			moveCakePiece(2,250,-135,"piece");
			break;
		case 3:
			moveCakePiece(3,200,-180,"piece");
			break;
		case 4:
			moveCakePiece(4,-150,-170,"piece");
			break;
		case 5:
			moveCakePiece(5,-200,0,"piece");
			break;
		default:
			break;
	}
...
function moveCakePiece(piecenumber,x,y,action) {
	...
	console.log("checking if piece should be moved");
	...
	if (cake.focus  && soundQueue.length==0) {
		...
		if (movePerformed && (x!=0 || y!=0)) {
			...
			if (selectedNameNumber==piecenumber) {	
				...
				extendAndPlayQueue(["det_var_vael_inte",names[selectedNameNumber]]);
				if (action=="character") {
					extendAndPlayQueue(["det_aer_ju",names[piecenumber],"som","faar",ordinal[movedPieces],"taartbiten"]);
				} else if (action=="piece") {
					extendAndPlayQueue(["det_aer_ju",ordinal[movedPieces],"taartbiten"]);
				}
			}
		}
		...
		if (!movePerformed && (x!=0 || y!=0) && selectedNameNumber!=piecenumber && selectedNameNumber!=-1) {
			...
			extendAndPlayQueue(["det_var_vael_inte",names[selectedNameNumber]]);
			if (action=="character") {
				extendAndPlayQueue(["det_aer_ju",names[piecenumber]]);
			}
		}
