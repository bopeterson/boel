...
*/	
	...
	function init() {"use strict";
		...
		*/
		...
		var simpleImageFiles = [
			...
			{id:"toreSad", src:"assets/toresad.png"},	
	...
	function changeBackground(color) {"use strict";
	...
	function createBackground(color,alpha) {"use strict";
	...
	function handleBackgroundTouch(event) {"use strict";
		if (debug) {
	...
	function printDebug(text) {"use strict";
	...
	function handleFileError(event) {"use strict";
		//var div = document.getElementById("loader");
		//div.innerHTML = "File load error";
		//div.style.backgroundColor = "#FF0000";
	}
		
	function handleFileLoad(event) {"use strict";
	...
	function updateProgressBar(progress) {"use strict";
		progressBar.scaleX = progress;
		stage.update();	
	}
	
	function addProgressBar() {"use strict";
	...
	function handleComplete(event) {"use strict";
	...
	function addBackground() {"use strict";
	...
	function handleTick(event) {"use strict";
	...
	function checkCharacter() {"use strict";
	...
	function handleNextBall() {"use strict";
	...
	function checkBallOutsideAndBallBounce() {"use strict";
	...
	function selectNextCharacter() {"use strict";
		var notMoved = [];
	...
	function smashNextPiece() {"use strict";
	...
	function makeBallTween(ball,x,y,speed) {"use strict";
	...
	function randomDirection() {"use strict";
		return (Math.random()<0.5)	? -1.0 : 1.0;
	}
	
	function randomSpeedAndDirection() {"use strict";
	...
	function isRunning(tween) {"use strict";
	...
	function handleCharacterTouch(event) {"use strict";
	...
	function handleCakePieceTouch(event) {"use strict";
	...
	function changeSplashScreen(alpha) {"use strict";
	...
	function showSplashScreen() {"use strict";
		changeSplashScreen(1.0);
	}
	
	function hideSplashScreen() {"use strict";
		changeSplashScreen(0.0);
	}
	
	function handleMenuHelpTouch(evt) {"use strict";
		if (noGameRunning()) {
	...
	function showHelp(text,sound) {"use strict";
	...
	function handleMenuCakeTouch(event) {"use strict";
		if (noGameRunning()) {
			menuCake.focus = true;
	...
	function handleBackButtonTouch(event) {"use strict";
	...
	function addDebugText() {"use strict";
	...
	function handleMenuBallTouch(event) {"use strict";
		if (noGameRunning()) {
			menuBall.focus = true;
			extendAndPlayQueue(["tyst1000"]);//very weird. this sound is needed for first sound to play on iphone.
			hideSplashScreen();
	...
	function noGameRunning() {"use strict";
		return (!menuBall.focus && !menuCake.focus && !menuClothes.focus);
	}
	
	function addClothes() {"use strict";
	...
	function handleMenuClothesTouch(event) {"use strict";
		if (noGameRunning()) {
	...
	function startClothesGame(delay) {"use strict";
	...
	function restorePieceOfClothes(c) {"use strict";
	...
	function restoreMenuClothes() {"use strict";
		menuClothes.focus = false;
	...
	function hideClothes() {"use strict";
		log("hideClothes()");
	...
	function showClothesGame() {"use strict";
	...
	function showBallGame() {"use strict";
	...
	function startBallGame(delay) {"use strict";
		hideStar();	
		...
		var i; for (i = 0;i<allBalls.length;i += 1) {
	...
	function restoreMenuBall() {"use strict";
	...
	function setRandomPosition(ball) {"use strict";
		var free = false;
	...
	function collides(ball1,ball2,forceField) {"use strict";
	...
	function findRandomBall(thisBall) {"use strict";
	...
	function detectBallCollision(thisBall,forceField) {"use strict";
		//klumpigt kanske, men funkar för bara fyra bollar.
		var collision = false;
		var otherBall = {};
		var i; for (i = 0;i<allBalls.length && !collision;i += 1) {
			if (allBalls[i]!= thisBall  && allBalls[i].inside) {
	...
	function handlePlayBallTouch(event) {"use strict";
	...
	function handleTableTouch(event) {"use strict";
		//console.log("xxx not used right now, but prevents from clicking through table");
	}
	
	function addCharacterEventListener(character) {"use strict";
	...
	function makeCharacterHappy(character) {"use strict";
		var c = character;
	...
	function makeCharacterSad(character) {"use strict";
		var c = character;
		if (c.hasOwnProperty("happyPic")) {
	...
	function addCake(pieceParts,numberOfCakePieces,cakeFiles) {"use strict";
		//these won't change
		...
		var onePiece;
		var part;
		var i,j; for (i = 0;i<numberOfCakePieces;i += 1) {
			onePiece = []; 
	...
	function handlePrePressmoveMousedown(evt) {"use strict";
		var t;
		var t2;
	...
	function handlePressmove(evt) {"use strict";
		var t;
		var t2;
		//make sure t is always linepic, and t2 is extraarea
		if (evt.target.kind=="linePic") {
	...
	function checkIfAllClothesOn() {"use strict";
	...
	function handlePostPressmovePressup(evt) {"use strict";
		var t;
		var t2;
		//make sure t is always linepic, and t2 is extraarea
		if (evt.target.kind=="linePic") {
	...
	function restoreMenuCake() {"use strict";
		...
		if (menuCake.focus) {
			...
			var i; for (i = 0;i<cakeComplete.length;i += 1) {
				restoreCakePiece(i);
			}
	...
	function addBackButton() {"use strict";
	...
	function buildCakeFiles(pieceParts,numberOfCakePieces) {"use strict";	
		...
		var k = 0;
		var i,j; for (i = 1;i<numberOfCakePieces + 1;i += 1) {
	...
	function restoreCakePiece(piecenumber) {"use strict";
		var piece = cakeComplete[piecenumber];
		
		var j; for (j = 0;j<piece.length;j += 1) {
			if (j==0 || j==2 || j==3) {
	...
	function moveCakePiece(piecenumber,x,y,action) {"use strict";
		...
		if (menuCake.focus  && soundQueue.length==0) {
			...
			var nextPiece;
			var prevPiece;
			var k; for (k = 0;k<cakeComplete.length;k += 1) {
	...
	function extendAndPlayQueue(sounds) {"use strict";
	...
	function playSingle(sound) {"use strict";
		var soundinstance;
		soundinstance = createjs.Sound.play(sound);
	}
	
	
	function handleNextSound(evt) {"use strict";
		soundQueue = soundQueue.splice(1);
		playQueue();
	}
	
	function watchSound(s0) {"use strict";
		var i; for (i = 0;i<ordinal.length;i += 1) {
			if (s0==ordinal[i]) {
				showNumber(i);	
			}
		}
	...
	function hideAllNumbers() {"use strict";
		//basically the same as hideNumber but immediate instead of tween
		var i; for (i = 0;i<numbers.length;i += 1) {
			numbers[i].alpha = 0;
		}
	}
	
	function hideNumber() {"use strict";
		var i; for (i = 0;i<numbers.length;i += 1) {
	...
	function showNumber(number) {"use strict";
		hideNumber();
		n = numbers[number];
	...
	function randomFutureMillis(minSec,maxSec) {"use strict";
	...
	function randomFutureMillis2(minSec,maxSec) {"use strict";
	...
	function bounceToTable(piecenumber) {"use strict";
	...
	function restoreCakeBall() {"use strict";
	...
	function smashPiece() {"use strict";
	...
	function getCharacterNameSound(pieceNumber) {"use strict";
	...
	function getCharacter(pieceNumber) {"use strict";
		var character = {};
	...
	function pulsate(bitmap,pulsetime) {"use strict";
	...
	function Ball(imageid,name,color) {"use strict";
	...
	function Clothes(linePicId,wearPicId,name,startRotation,startX,startY,wearX,wearY,movable) {"use strict";
	...
	function Character(pieceNumber,pieceDeltaX,pieceDeltaY,happyPic,hasSadPic,hasHandPic,name,regX,regY) {"use strict";
	...
	//drawing functions
	function enableDrawing() {"use strict";
		if (helpContainer.visible) {
			...
			var i; for (i = 0;i<crayonColors.length;i += 1) {
	...
	function disableDrawing() {"use strict";
		clothesGameContainer.drawingEnabled = false;
		menuHelp.alpha = 1.0;
		tore.sad.alpha = 0;
		clothesGameContainer.clean = true;
	...
	function removeCrayons() {"use strict";
		var i; for (i = 0;i<crayonColors.length;i += 1) {
	...
	function makeCrayon(color,outlineColor,outlineWidth) {"use strict";
	...
	function handleClothesPaint() {"use strict";
	...
	function drawCrayons(index) {"use strict";
	...
	function handleCrayonMouseDown(evt) {"use strict";
	...
	function handleStrokeMouseDown(evt) {"use strict";
	...
	function handleStrokeMouseMove(evt) {"use strict";
	...
	function handleStrokeMouseUp(evt) {"use strict";
	...
	function detectPaint() {"use strict";
		var detected = false;
		var pt;
		var i; for (i = 0;i<paintDetect.length && !detected;i += 1) {
